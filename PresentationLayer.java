...
public class PresentationLayer {
    ...
    private static void login() {
        ...
        try {
            ...
            if (account.accountType.equals("Faculty")) {
                ...
                if (faculty == null) { error("Faculty profile not found!"); return; }
                showFacultyMenu();
            } else if (account.accountType.equals("Student")) {
                student = db.getStudentByAccount(account.accountId);
                if (student == null) { error("Student profile not found!"); return; }
                showStudentMenu();
            } else {
                error("Guest accounts not supported in this version.");
                return;
            }
    ...
    private static void addAbstract() {
        ...
        String[] labels = {"Title:", "Type:", "Authors:"};
        ...
        if (method == 0) {
            ...
            gbc.gridx = 0; gbc.gridy = 4; p.add(new JLabel("File Path:"), gbc);
            ...
            try {
                String content = new String(Files.readAllBytes(Paths.get(pathField.getText().trim())));
            ...
            gbc.gridx = 0; gbc.gridy = 4; p.add(new JLabel("Abstract Text:"), gbc);
    ...
    private static void deleteAbstract() {
        try {
            ...
            if (JOptionPane.showConfirmDialog(null, "Delete \"" + abs.title + "\"?", "Confirm", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
                db.deleteAbstract(faculty.facultyId, abs.abstractId);
                success("Abstract deleted!");
            }
        } catch (Exception e) { error("Error: " + e.getMessage()); }
    }
